...
class Episode:
    ...
    def __init__(self, jsonData, sessData, comicID):
        ...
        # header头 模拟浏览器 溯源 防盗链反爬 个人cookies
        self.headers = {
    ...
    # 下载单页漫画 retey装饰器用于出错时重新下载
    @retry(delay=1)
    def downloadImg(self, token, url, index):
        url = url + "?token=" + token
        # b站的反爬措施并不理想 下载并不需要伪装头
    ...
    def download(self):
        # 获取单章节全部漫画的url 参数带上ep_id 也就是每章节漫画的id
        ...
        if rep.ok:
            # 直接获取json数据
            data = rep.json()
            # 提取出每页漫画的 url 用于下一步请求获取每页漫画的token值
            images = data['data']['images']
            paths = []
            for img in images:
                paths.append(img['path'])

            # json数据转化为一个列表 装满每页漫画的url
            payloads = {
...
# 关于bilbil漫画判断是否购买是使用一个is_locked字段 True Flase
# ?是否可以提前拦截该请求进行更改从而获取全部漫画章节 深入学习后可以尝试
